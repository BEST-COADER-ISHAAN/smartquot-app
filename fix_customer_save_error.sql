/*
  # Fix Customer Save Error - Complete Auth0 Integration Fix

  This script fixes the "Failed to save customer. Please try again." error
  by ensuring proper Auth0 integration and RLS policies.

  Problems:
  1. created_by field type mismatch (UUID vs TEXT)
  2. RLS policies too restrictive for Auth0 users
  3. Missing or incorrect triggers for Auth0 integration

  Solutions:
  1. Ensure created_by fields are TEXT type
  2. Create permissive RLS policies for authenticated users
  3. Add proper triggers for Auth0 integration
*/

-- Step 1: Fix created_by field types (ensure they are TEXT)
DO $$
DECLARE
    current_table text;
    column_type text;
...
-- Step 3: Drop ALL existing policies and create simple permissive ones
-- Customers table
...
DROP POLICY IF EXISTS "Authenticated users can manage quotation rooms" ON quotation_rooms;
...
DROP POLICY IF EXISTS "Authenticated users can manage quotation room items" ON quotation_room_items;
...
-- Step 4: Create trigger function for Auth0 integration
...
BEGIN
    ...
    RAISE NOTICE '=== CUSTOMER SAVE ERROR FIX COMPLETE ===';
    ...
    RAISE NOTICE '✅ Customer saving should now work correctly';
    RAISE NOTICE '✅ All CRUD operations should work without issues';
END $$;
