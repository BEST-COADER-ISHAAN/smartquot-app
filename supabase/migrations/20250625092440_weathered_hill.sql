/*
  # Fix quotation numbering system with format #QT[4-digit customer number][letter]
  ...
  2. Quotation Number Format
    - #QT = prefix
    - 0100 = customer number (4 digits, starting from 0100)
...
BEGIN
    ...
    IF customer_uuid IS NULL THEN
        SELECT COALESCE(MAX(CAST(SUBSTRING(quotation_number FROM 4 FOR 4) AS integer)), 99) + 1
...
DECLARE
    ...
    next_customer_number integer := 100; -- Start from 0100
BEGIN
    ...
    SELECT DISTINCT 
        customer_id,
        LPAD((ROW_NUMBER() OVER (ORDER BY MIN(created_at)) + 99)::text, 4, '0')
    FROM quotations 
