/*
  # Enable Row-Level Security with User Isolation (Fixed Version)
  
  This script enables RLS on all tables with proper user isolation for Auth0 integration.
  Fixed the JOIN syntax error in quotation_room_items policies.
*/
...
-- Step 7: Create user isolation policies for quotation_room_items (FIXED)
CREATE POLICY "Users can read own quotation room items"
  ON quotation_room_items FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM quotation_rooms qr
      JOIN quotations q ON q.id = qr.quotation_id
      WHERE qr.id = quotation_room_items.room_id 
      AND (
        q.created_by = current_setting('app.auth0_user_id', true)::text
...
CREATE POLICY "Users can update own quotation room items"
  ...
  USING (
    EXISTS (
      ...
      JOIN quotations q ON q.id = qr.quotation_id
      WHERE qr.id = quotation_room_items.room_id 
      AND q.created_by = current_setting('app.auth0_user_id', true)::text
    )
  );
