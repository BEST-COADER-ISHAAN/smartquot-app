/*
  # Complete Auth0 Integration Fix (Safe Version)

  This script fixes both the created_by field type and RLS policies for Auth0 integration.
  This version safely handles existing policies and won't fail if they already exist.
...
-- Step 1: Fix created_by field types (only if needed)
DO $$
DECLARE
    column_type text;
BEGIN
...
-- Step 2: Safely drop and recreate RLS policies
-- Products table
...
-- Step 4: Safely create triggers for all tables
...
BEGIN
    ...
    FOR table_name IN SELECT unnest(ARRAY['customers', 'products', 'quotations']) LOOP
        ...
        IF column_type = 'text' THEN
            RAISE NOTICE 'SUCCESS: % table created_by field is TEXT type', table_name;
        ELSE
